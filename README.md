...
Common options: `--max-pages` (50), `--max-depth` (3), `--timeout` (15 seconds per socket operation), `--delay` (0.25 seconds minimum between requests to an origin), `--max-bytes` (2,000,000 per response), `--max-sitemaps` (20), `--max-urls` (10,000 discovery/queue limit), `--company-name`, repeatable `--known-profile`, `--collect-social`, repeatable `--dimension` (see [Dimensions](#dimensions)). Domain-only input defaults to HTTPS. Robots crawl delays are honored when parseable by Python's robots parser.
...
- Social discovery uses website links, JSON-LD sameAs and selected OpenGraph fields. No external search, platform API or recent-post adapter is built in. Discovered accounts remain unverified. Optional public HTML captures may be login/challenge screens and require review; social failure never invalidates the website evidence.
...
| `COMPANYSCAN_REPUTATION_MODEL` | `llm_reputation` and `companyscan reputation` (~50 short calls) | `openai:gpt-4o-mini` |
...
Renders the newest `analysis/**/report.md` to `report.html` and `report.pdf` beside it: pandoc (Markdown to HTML) with `src/companyscan/views/report/report.css`, then headless Chrome prints it. PASS/WARNING/FAIL/UNKNOWN and HIGH/MEDIUM/LOW become coloured badges. It needs `pandoc` and Chrome or Chromium (set `CHROME=/path/to/chrome` if it isn't found). To restyle every report, edit `report.css`.
